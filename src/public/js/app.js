...
function setupIndex() {
  ...
  // load JSON data
  fetch('http://localhost/data/get-top-books').then(function (response) {
...
function setupSingle(id) {
  ...
  // load JSON data
  fetch('http://localhost/data/get-book/' + id).then(function (response) {
    return response.json();
  }).then(function (book) {
    ...
    // load related books
    fetch('http://localhost/data/get-related-books/' + id).then(function (response) {
      return response.json();
    }).then(function (books) {
...
function renderRelated(books) {
  ...
  title.classList = 'display-4';
  title.textContent = "LÄ«dzÄ«gas grÄmatas";
  ...
  var row = document.createElement('div');
  row.classList = 'row mb-5';
  var _iterator2 = _createForOfIteratorHelper(books),
    _step2;
  try {
